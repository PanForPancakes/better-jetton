...
export function jettonContentToCell(content:MinterContent) {
    return beginCell()
...
export class Minter implements Contract {
    ...
    protected static jettonInternalTransfer(jetton_amount: bigint, forward_ton_amount: bigint, response_addr?: Address, query_id: number | bigint = 0) {
        return beginCell()
    ...
    async sendMint(provider: ContractProvider, via: Sender, to: Address, jetton_amount: bigint, forward_ton_amount: bigint, total_ton_amount: bigint) {
        ...
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: Minter.mintMessage(this.address, to, jetton_amount, forward_ton_amount, total_ton_amount),
    ...
    */
    ...
    async sendChangeAdmin(provider: ContractProvider, via: Sender, newOwner: Address) {
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: Minter.changeAdminMessage(newOwner),
            value: toNano("0.05"),
        });
    }
    ...
    async sendChangeContent(provider: ContractProvider, via: Sender, content: Cell) {
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: Minter.changeContentMessage(content),
            value: toNano("0.05"),
        });
    }
