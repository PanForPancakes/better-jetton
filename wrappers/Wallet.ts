...
export class Wallet implements Contract {
    ...
    async sendTransfer(provider: ContractProvider, via: Sender,
        ...
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: Wallet.transferMessage(jetton_amount, to, responseAddress, customPayload, forward_ton_amount, forwardPayload),
            value:value
        });
    ...
    */
    ...
    async sendBurn(provider: ContractProvider, via: Sender, value: bigint,
        ...
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: Wallet.burnMessage(jetton_amount, responseAddress, customPayload),
            value:value
        });
    ...
    */
    ...
    async sendWithdrawTons(provider: ContractProvider, via: Sender) {
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: Wallet.withdrawTonsMessage(),
            value:toNano('0.1')
        });
    ...
    */
    ...
    async sendWithdrawJettons(provider: ContractProvider, via: Sender, from:Address, amount:bigint) {
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: Wallet.withdrawJettonsMessage(from, amount),
            value:toNano('0.1')
        });
