...
export const getAccountLastTx = async (provider: NetworkProvider, address: Address) => {
    const res = await provider.provider(address).getState();
    if(res.last == null)
        throw(Error("Contract is not active"));
    return res.last.lt;
}
export const waitForTransaction = async (provider:NetworkProvider, address:Address, curTx:bigint | null, maxRetry:number, interval:number=1000) => {
    let done  = false;
    let count = 0;
